...
import skmob
from skmob.measures.individual import jump_lengths, radius_of_gyration, random_entropy, uncorrelated_entropy, real_entropy
...
def preprocess_staypoints(bmwdf, stpgdf, mob_folder):
    ...
    # DBSCAN clustering on user level - 68094 locations
    stp_trackintel = trackintel.read_staypoints_gpd(stpgdf, started_at='started_at', finished_at='finished_at', user_id="user_id", geom="geometry_raw")
...
def extract_mobility_daily(stp_cls, userlist, PREPROCESS_PATH):
    ...
    for user in userlist:
        ...
        # find top 3 locations for the user
        locslists = stp_cls_user['location_id'].value_counts()[:].index.tolist()
        if len(locslists) >= 3:
            topk_user = stp_cls_user['location_id'].value_counts().index.tolist()[:3]
        else:
            topk_user = stp_cls_user['location_id'].value_counts()[:].index.tolist()
        ...
        while start_date <= end_date:
            ...
            if exist_flag != 0:
                # calculate top-3 location frequency
